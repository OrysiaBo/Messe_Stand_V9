...
class CreatorTab:
    ...
    def __init__(self, parent, main_window):
        ...
        self.canvas_data_file = "canvas_elements.json"
        self.canvas_elements_storage = {}
    ...
    def save_canvas_elements(self, slide_id, elements):
        """Зберігає canvas елементи в окремий файл"""
        try:
            if os.path.exists(self.canvas_data_file):
                with open(self.canvas_data_file, 'r', encoding='utf-8') as f:
                    self.canvas_elements_storage = json.load(f)
            
            self.canvas_elements_storage[str(slide_id)] = elements
            
            with open(self.canvas_data_file, 'w', encoding='utf-8') as f:
                json.dump(self.canvas_elements_storage, f, indent=2, ensure_ascii=False)
            
            print(f"DEBUG: Saved {len(elements)} canvas elements for slide {slide_id}")
            return True
        except Exception as e:
            logger.error(f"Error saving canvas elements: {e}")
            return False
    
    def load_canvas_elements(self, slide_id):
        """Завантажує canvas елементи з файлу"""
        try:
            if os.path.exists(self.canvas_data_file):
                with open(self.canvas_data_file, 'r', encoding='utf-8') as f:
                    self.canvas_elements_storage = json.load(f)
                
                elements = self.canvas_elements_storage.get(str(slide_id), [])
                print(f"DEBUG: Loaded {len(elements)} canvas elements for slide {slide_id}")
                return elements
            return []
        except Exception as e:
            logger.error(f"Error loading canvas elements: {e}")
            return []
    ...
    def save_current_slide_content(self):
        ...
        try:
            ...
            if self.edit_mode and hasattr(self, 'edit_widgets'):
                ...
                # Зберегти також canvas елементи в режимі редагування
            ...
            print(f"DEBUG: Found {len(canvas_elements)} canvas elements to save")
            for element in canvas_elements:
                print(f"DEBUG: Element type: {element.get('type')}, x: {element.get('x')}, y: {element.get('y')}")
            
            # Зберегти через content_manager для синхронізації тексту
            success1 = content_manager.update_slide_content(
                self.current_edit_slide,
                title_text,
                content_text
            )
            
            # Зберегти canvas елементи окремо
            success2 = self.save_canvas_elements(self.current_edit_slide, canvas_elements)
            
            success = success1 and success2
            print(f"DEBUG: Save success: text={success1}, canvas={success2}, overall={success}")
    ...
    def extract_image_data(self, widget, coords):
        """Витягує дані картинки з widget"""
        try:
            image_data = {
                'type': 'image',
                'x': coords[0] if coords else 0,
            ...
            if hasattr(widget, 'image_path'):
                ...
                print(f"DEBUG: Saving image with file path: {widget.image_path}")
            
            # Зберегти оригінальну картинку як base64
            if hasattr(widget, 'original_image'):
                try:
                    buffer = BytesIO()
                    widget.original_image.save(buffer, format='PNG')
                    image_base64 = base64.b64encode(buffer.getvalue()).decode('utf-8')
                    image_data['image_data'] = image_base64
                    print(f"DEBUG: Saved image as base64, size: {len(image_base64)} chars")
                except Exception as e:
    ...
    def load_slide_to_editor(self, slide_id):
        ...
        try:
            ...
            if slide:
                ...
                # Завантажити збережені canvas елементи
                elements = self.load_canvas_elements(slide_id)
                if elements:
                    print(f"DEBUG: Restoring {len(elements)} canvas elements for slide {slide_id}")
                    self.restore_canvas_elements(elements)
                else:
    ...
    def restore_canvas_elements(self, canvas_elements):
        ...
        try:
            ...
            for element in canvas_elements:
                print(f"DEBUG: Restoring element type: {element.get('type')}")
    ...
    def restore_image_element(self, image_data):
        ...
        try:
            ...
            if 'file_path' in image_data and os.path.exists(image_data['file_path']):
                try:
                    print(f"DEBUG: Loading image from file: {image_data['file_path']}")
            ...
            if image is None and 'image_data' in image_data:
                try:
                    print(f"DEBUG: Loading image from base64 data")
            ...
            if image:
                ...
                image_label.original_image = image  # Зберегти оригінал
                ...
                print(f"DEBUG: Restored image at position ({image_data['x']}, {image_data['y']})")
                logger.debug(f"Restored image at position ({image_data['x']}, {image_data['y']})")
            else:
                print(f"DEBUG: Failed to restore image - no valid image data")
            
        except Exception as e:
            logger.error(f"Error restoring image element: {e}")
            print(f"DEBUG: Error restoring image: {e}")
    ...
    def toggle_edit_mode(self):
        ...
        try:
            ...
            if self.edit_mode:
                ...
                self.save_current_slide_content()
                self.clear_slide_canvas()
                self.load_slide_to_editor(self.current_edit_slide)  # Перезавантажити з елементами
    ...
    def create_edit_widgets(self):
        ...
        try:
            ...
            # Також відновити збережені canvas елементи в режимі редагування
            elements = self.load_canvas_elements(self.current_edit_slide)
            if elements:
                for element in elements:
                    if element['type'] == 'image':
                        self.restore_image_element(element)
                    # Не відновлювати додаткові текстові елементи в режимі редагування основного тексту
    ...
    def add_image_element(self):
        ...
        try:
            ...
            if file_path:
                ...
                image_label.original_image = image  # Зберегти оригінальну картинку
                ...
                print(f"DEBUG: Image added to canvas")
    ...
    def clear_slide(self):
        ...
        try:
            ...
            if result:
                ...
                self.save_canvas_elements(self.current_edit_slide, [])
